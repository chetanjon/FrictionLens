    ...
    <SectionWrapper id="summary">
      ...
      <div className="flex flex-wrap items-start gap-10">
        <div className="min-w-[300px] flex-1">
          <div className="mb-5 flex items-center gap-3.5">
            <div
              ...
              style={{
                ...
                boxShadow: "0 6px 20px rgba(74,144,217,0.15)",
              }}
            >
              <span className="leading-none text-white">{initial}</span>
            </div>
            <div>
              <h1 className="m-0 text-[28px] font-extrabold tracking-[-1px] text-slate-900">
                {appName}
              </h1>
          ...
          <p className="mb-5 max-w-[480px] text-[15.5px] leading-7 text-slate-500">
            {summary}
          </p>
        </div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {/* Churn Risk */}
        <div className="rounded-2xl border border-slate-200/60 bg-white/65 px-4 py-[18px] backdrop-blur-xl">
          ...
          <div
            ...
            style={{ color: (churnRiskPercent ?? 0) > 15 ? "#D94F4F" : (churnRiskPercent ?? 0) > 8 ? "#D4A843" : "#4A90D9" }}
          >
        ...
        <div className="rounded-2xl border border-slate-200/60 bg-white/65 px-4 py-[18px] backdrop-blur-xl">
          ...
          <div className="mt-1 font-mono text-2xl font-extrabold tracking-tight text-[#D94F4F]">
        ...
        <div className="rounded-2xl border border-slate-200/60 bg-white/65 px-4 py-[18px] backdrop-blur-xl">
          ...
          <div
            ...
            style={{ color: releaseGrade ? "#D4A843" : "#94A3B8" }}
          >
            {releaseGrade ?? "—"}
          </div>
          {releaseVersion && (
      ...
      {churnRiskPercent != null && churnRiskPercent > 0 && topFriction && (
        <div className="mt-4 flex items-center gap-4 rounded-2xl border border-red-200/60 bg-red-50/70 px-5 py-4 backdrop-blur-xl">
          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-red-100">
            <svg className="h-5 w-5 text-[#D94F4F]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.072 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
          </div>
          <div className="flex-1">
            <div className="font-mono text-[10px] font-semibold uppercase tracking-[1.5px] text-[#D94F4F]">
              Top Churn Driver
            </div>
          ...
          <span className="shrink-0 rounded-full bg-[#D94F4F] px-3 py-1 font-mono text-xs font-bold text-white">
            P0
          </span>
        </div>
      )}
  ...
  return (
    <svg viewBox="0 0 260 248" className="mx-auto w-full max-w-[260px]">
      {[2.5, 5, 7.5, 10].map((level) => {
        ...
        return <polygon key={level} points={gp.map((p) => p.join(",")).join(" ")} fill="none" stroke="#E2E8F0" strokeWidth="0.7" />;
      })}
      {Array.from({ length: 5 }, (_, i) => {
        const [ex, ey] = toXY(10, i);
        return <line key={i} x1={cx} y1={cy} x2={ex} y2={ey} stroke="#E2E8F0" strokeWidth="0.5" />;
      })}
      <defs>
      ...
      <polygon points={pts.map((p) => p.join(",")).join(" ")} fill="url(#radar-mini-fill)" stroke="#4A90D9" strokeWidth="1.5" />
      {pts.map(([x, y], i) => (
...
function FrictionMiniChart({ items }: { items: Array<{ name: string; score: number }> }) {
  function barColor(s: number): string {
    if (s > 7) return "#D94F4F";
    if (s > 4) return "#D4A843";
    return "#4A90D9";
  }
    ...
    <div className="space-y-2.5 py-1">
      {items.map((item) => (
        <div key={item.name} className="flex items-center gap-3">
          <span className="w-[120px] shrink-0 truncate text-[13px] font-medium text-slate-700">
            {item.name}
          </span>
          <div className="h-[10px] flex-1 rounded bg-slate-100">
            <div
